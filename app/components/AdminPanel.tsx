...
export function AdminPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <>
    {open && <div className="fixed inset-0 z-[70] overflow-y-auto bg-slate-950/70 p-4 backdrop-blur-sm"><section className="mx-auto max-w-2xl rounded-2xl bg-white p-6 text-slate-900 shadow-2xl">
      ...
      <form onSubmit={createCompany} className="mt-5 grid gap-2 sm:grid-cols-4"><input required value={name} onChange={e=>setName(e.target.value)} placeholder="Nombre de empresa" className="rounded border p-2"/><input value={rnc} onChange={e=>setRnc(e.target.value)} placeholder="RNC (opcional)" className="rounded border p-2"/><input min="1" type="number" value={monthlyLimit} onChange={e=>setMonthlyLimit(e.target.value)} placeholder="Límite mensual" className="rounded border p-2"/><button className="rounded bg-violet-600 px-3 py-2 font-semibold text-white">Crear empresa</button></form>
      ...
      <h3 className="mt-6 font-semibold">Empresas</h3><div className="mt-2 space-y-2">{companies.map(c=>{const used = usage.find(u=>u.company_id === c.id)?.invoice_count ?? 0; const limitText = c.monthly_invoice_limit === null ? "Sin límite" : `${used.toLocaleString("es-DO")} / ${c.monthly_invoice_limit.toLocaleString("es-DO")} facturas este mes`; return <div key={c.id} className="flex flex-wrap items-center gap-2 rounded border p-3 text-sm"><span className="min-w-40 flex-1"><b>{c.name}</b><br/>Vence: {new Intl.DateTimeFormat("es-DO", {dateStyle:"medium"}).format(new Date(c.license_expires_at))}<br/><span className="text-slate-600">Límite: {limitText}</span></span><button onClick={()=>void editCompany(c)} className="rounded bg-sky-600 px-3 py-2 font-semibold text-white">Editar</button><button onClick={()=>void editMonthlyLimit(c)} className="rounded bg-violet-600 px-3 py-2 font-semibold text-white">Límite</button><button onClick={()=>void deleteCompany(c)} className="rounded bg-rose-600 px-3 py-2 font-semibold text-white">Eliminar</button><button onClick={()=>void renew(c.id)} className="rounded bg-emerald-600 px-3 py-2 font-semibold text-white">Marcar pagado · 30 días</button></div>})}</div>
    </section></div>}
  </>;
}
