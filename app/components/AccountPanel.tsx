...
export function AccountPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  async function changePassword(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (password.length < 8) { setIsError(true); setMessage("La contraseña debe tener al menos 8 caracteres."); return; }
    if (password !== confirmation) { setIsError(true); setMessage("Las contraseñas no coinciden."); return; }
    ...
    setSaving(false);
    if (!response.ok) { setIsError(true); setMessage(result.error ?? "No se pudo cambiar la contraseña. Intenta nuevamente."); return; }
    setPassword("");
    setConfirmation("");
